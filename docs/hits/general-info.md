...
All of the hits you obtain with a *Multi Run Job* that has *Database* in its [hit outputs](./hit-outputs.md) will be saved to the SQLite database, usually located at `UserData/OpenBullet2.db`. You can view them in the *Hits* section of OpenBullet 2.

### Types of hits
Hits have a type that depends on the *status* of the bot when it ended the execution of the config. Configs can change the status of the bot that executes them through *Keycheck* blocks or statements that specifically set the value of the `data.STATUS` variable. The status values that will result in a hit being sent to the hit outputs (for example the database) are, by default:
...
- **Download with format** - downloads a text file with the hits inside, in the chosen format
...
:::info INFO
The wordlist is taken into consideration when deduplicating hits. If you wish to ignore it, go to the *OB Settings* page and enable *Ignore the wordlist name when removing duplicates from hits*.
:::
