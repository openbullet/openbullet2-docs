...
Go to the *Wordlists* section of OpenBullet 2 and click on the *Add* button. There are two ways to import a wordlist: the first is by uploading it through the browser, and the second is by selecting it from the file system.
...
You should use this option if you are hosting OpenBullet 2 **remotely**. In this case, the only way to send a wordlist from your local device to the server where OpenBullet 2 is running is by uploading it through the browser. The wordlist will be copied inside the `UserData/Wordlists` folder and will be given a randomly generated name.
...
Finally, select the *Wordlist Type* (more on wordlist types [here](./general-info.md)), give your wordlist a *name*, and add a *purpose* to remind yourself what the wordlist is used for.
